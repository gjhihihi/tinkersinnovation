...
public record BombShellStats(float bomb_radius, int piece_count, float piece_damage, float melee_range_damage) implements IMaterialStats {
    ...
    public static final MaterialStatType<ShieldMaterialStats> TYPE = new MaterialStatType(
            ID,
            new BombShellStats(1, 1, 1, 1),
            RecordLoadable.create(
                    FloatLoadable.FROM_ZERO.defaultField("bomb_radius",1f, true, BombShellStats::bomb_radius),
                    IntLoadable.FROM_ZERO.defaultField("piece_count",1, true, BombShellStats::piece_count),
                    FloatLoadable.FROM_ZERO.defaultField("piece_damage",1f, true, BombShellStats::piece_damage),
                    FloatLoadable.FROM_ZERO.defaultField("melee_range_damage",1f, true, BombShellStats::melee_range_damage),
                    BombShellStats::new
            )
    );
    private static final List<Component> DESCRIPTION = ImmutableList.of(
            TinkersInnovationToolStats.BOMB_RADIUS.getDescription(),
            TinkersInnovationToolStats.PIECE_COUNT.getDescription(),
            TinkersInnovationToolStats.PIECE_DAMAGE.getDescription(),
            ToolStats.ATTACK_DAMAGE.getDescription(),
            ToolStats.PROJECTILE_DAMAGE.getDescription()
    );
