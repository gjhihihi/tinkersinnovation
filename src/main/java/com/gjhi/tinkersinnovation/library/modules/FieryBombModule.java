...
public record FieryBombModule(LevelingValue time) implements ModifierModule, TinkersBombHook {
    public static final RecordLoadable<FieryBombModule> LOADER = RecordLoadable.create(
            LevelingValue.LOADABLE.requiredField("seconds", FieryBombModule::time),
            FieryBombModule::new
    );
    ...
    public void afterTinkersBombExplode(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, EBomb bomb, LivingEntity attacker, List<LivingEntity> targets) {
        for (LivingEntity target : targets){
            target.setSecondsOnFire(Math.round(time.compute(modifier.getEffectiveLevel()) * modifier.getLevel()));
        }
    }
